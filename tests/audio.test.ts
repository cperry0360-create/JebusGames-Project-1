...
test('the CC0 audio is credited', () => {
  ...
  const names = credits.sections.flatMap((s: any) => s.entries).map((e: any) => e.name).join(' ')
  assert.match(names, /Kenney/, 'the credits screen should name Kenney for the audio too')
})
