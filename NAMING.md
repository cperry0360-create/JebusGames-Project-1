...
Nothing here has been renamed. Fill in the **NEW NAME** column and the rename
...
|---|---|---|---|---|
| Tower | **Withholding Tower** | Loadout card, build menu, tower panel, all build/sell/upgrade messages | `11 damage · Short reach · 1.5/sec` / `Cuts 5 armour.` | |
| Tower tier 2 | *(unnamed)* | Tower panel: "TIER 2 OF 3" | — | |
| Tower tier 3 branch | **Garnishment** | Specialize dialog, tower panel | Damage ×1.9, ignores armour entirely | |
| Tower tier 3 branch | **Payroll** | Specialize dialog, tower panel | Faster fire, each shot chains to 1 extra target | |
| Tower | **Write-Off** | Loadout card, build menu, tower panel | `44 damage · Fair reach · 0.6/sec` / `Ignores armour entirely.` | |
| Tower tier 3 branch | **Total Loss** | Specialize dialog, tower panel | Damage ×2.2, executes anything under 18% health | |
| Tower tier 3 branch | **Carryforward** | Specialize dialog, tower panel | Damage ramps +14% per shot at the same target | |
| Tower | **Rounding Error** | Loadout card, build menu, tower panel | `9 damage · Short reach · 0.9/sec` / `Hits a small area.` | |
| Tower tier 3 branch | **Bankers** | Specialize dialog, tower panel | Bigger splash, splash also slows | |
| Tower tier 3 branch | **Materiality** | Specialize dialog, tower panel | Damage ×2.3, ×1.9 again against anything armoured | |
| Tower | **Escalation Clause** | Loadout card, build menu, tower panel | `27 damage · Fair reach · 0.4/sec` / `Hits a wide area.` | |
| Tower tier 3 branch | **Compound** | Specialize dialog, tower panel | Damage ramps +22% per shot, up to +160% | |
| Tower tier 3 branch | **Penalty** | Specialize dialog, tower panel | Wider splash, chains to 2 more targets | |
| Tower | **Filing Extension** | Loadout card, build menu, tower panel | `8 damage · Short reach · 1.1/sec` / `Slows what it hits to 45%, cuts 3 armour.` | |
| Tower tier 3 branch | **Deferral** | Specialize dialog, tower panel | Longer slow, chains the slow to 1 more target | |
| Tower tier 3 branch | **Amendment** | Specialize dialog, tower panel | Damage ×2.6, freezes the target for 0.6s | |
| Tower | **Tax Shelter** | Loadout card, build menu, tower panel | `Huge area · +30% damage` / `Buffs nearby towers. Cannot attack.` | |
| Tower tier 3 branch | **Offshore** | Specialize dialog, tower panel | Much bigger radius, neighbours also gain +20% range | |
| Tower tier 3 branch | **Loophole** | Specialize dialog, tower panel | Stronger buff, neighbours also gain 5 armour pierce | |

## Enemies

| CATEGORY | CURRENT NAME | WHERE IT APPEARS | CURRENT DESCRIPTION TEXT | NEW NAME |
|---|---|---|---|---|
| Enemy | **Late Filer** | Damage/kill feedback only — the name is not shown on screen during play | `Shambling, penalised, and still technically compliant.` *(flavour is in the data but is never displayed for non-boss enemies)* | |
| Enemy | **Shredder** | As above | `Fast, loud, and legally required to destroy the evidence.` *(not displayed)* | |
| Enemy | **Final Notice** | As above | `Bold red lettering. Structurally reinforced. Deeply unreasonable.` *(not displayed)* | |

## Boss

| CATEGORY | CURRENT NAME | WHERE IT APPEARS | CURRENT DESCRIPTION TEXT | NEW NAME |
|---|---|---|---|---|
| Boss | **The Politician** | Wave 13 name, the boss arrival card (name in 56px), the boss health bar, and every tax message | Card: `Here to listen. Here to help. Here to take a percentage.` <br> Arrival: `The Politician is here. He does not attack — he taxes. Spend your peanuts.` <br> Each tax: `The Politician taxed you N peanuts. Spend it or lose it.` | |

## Hero

| CATEGORY | CURRENT NAME | WHERE IT APPEARS | CURRENT DESCRIPTION TEXT | NEW NAME |
|---|---|---|---|---|
| Hero | **Cory** | Loadout hero card, HUD hero bar, every hero message | `Holds the line in person and shreds armour nearby.` | |
| Hero title | **The Optimizer** | **Nowhere.** It is in `heroes.json` but no screen renders it | — | |

## Hero abilities

| CATEGORY | CURRENT NAME | WHERE IT APPEARS | CURRENT DESCRIPTION TEXT | NEW NAME |
|---|---|---|---|---|
| Hero passive | **Depreciation** | **Nowhere.** In `heroes.json`; the radius ring that used to show it was deliberately removed | Shreds up to 7 armour from anything near him | |
| Hero active | **Haymaker** | Ability bar medallion, cast banner, refusal messages | `Haymaker!` on cast; `Haymaker: nothing in reach.` / `Haymaker is still recharging.` | |
| Hero active | **Restructure** | Ability bar medallion (DAD MODE only), all its prompts | `Restructure: click a tower, then a free spot.` · `Restructure needs DAD MODE.` · `<Tower> restructured. Back in 22s.` | |
| Hero ultimate state | **DAD MODE** | HUD hero bar (`· DAD MODE`), the transform, Restructure's gate | `DAD MODE is over. Restructure with it.` | |

## Specials and consumables

| CATEGORY | CURRENT NAME | WHERE IT APPEARS | CURRENT DESCRIPTION TEXT | NEW NAME |
|---|---|---|---|---|
| Special | **Molotov** | Loadout SPECIALS card, ability bar, cast banner | `85 damage, wide area · 14s cooldown` | |
| Special | **Gnomes** | As above | `2 blockers for 18s · 26s cooldown` | |
| Special | **Glacier** | As above | `12 damage, huge area · slows to 25% for 5s · 20s cooldown` | |
| Special | **Meteor** | As above | `36 damage, wide area · 30s cooldown` | |
| Special | **Chain** | As above | `46 damage, huge area · 18s cooldown` | |
| Special | **Scratch Ticket** | As above, plus the scratch card overlay | `up to 900 peanuts · 25% pay nothing · 34s cooldown` <br> Result: `Scratch Ticket: N peanuts.` / `Scratch Ticket: not a winner. Keep your day job.` | |
| Special | **Server Nuke** | Rare mid-run drop: earned overlay, launch confirmation, ability bar | `Deletes every enemy on the board · 0s cooldown` | |
...
| CATEGORY | CURRENT NAME | WHERE IT APPEARS | CURRENT DESCRIPTION TEXT | NEW NAME |
|---|---|---|---|---|
| Wave 1 | **Paperwork** | Wave banner, HUD | 4 Late Filers | |
| Wave 2 | **More Paperwork** | Wave banner, HUD | 6 Late Filers | |
| Wave 3 | **Deadline** | Wave banner, HUD | Late Filers + first Shredders | |
| Wave 4 | **Certified Mail** | Wave banner, HUD | Late Filers + first Final Notice | |
| Wave 5 | **Audit Season** | Wave banner, HUD | 12 Shredders + Late Filers | |
| Wave 6 | **Registered Post** | Wave banner, HUD | 5 Final Notices + Shredders | |
| Wave 7 | **Quarterly Filing** | Wave banner, HUD | All three types | |
| Wave 8 | **Collections** | Wave banner, HUD | All three types | |
| Wave 9 | **Second Notice** | Wave banner, HUD | All three types | |
| Wave 10 | **Penalty Interest** | Wave banner, HUD | All three types | |
| Wave 11 | **Levy** | Wave banner, HUD | All three types | |
| Wave 12 | **Final Assessment** | Wave banner, HUD | All three types | |
| Wave 13 | **The Politician** | Wave banner, HUD | The boss | |
...
| CATEGORY | CURRENT NAME | WHERE IT APPEARS | CURRENT DESCRIPTION TEXT | NEW NAME |
|---|---|---|---|---|
| Currency | **peanuts** | The HUD counter, every cost and refusal message, the results dialog, the scratch card, the sign bribe, and a whole credits department | `<Tower> costs 80 peanuts — 30 short.` · `Sold for 48 peanuts.` · `Peanuts earned` · `He wants more peanuts.` | |
| Meta currency | **Banner Points** | Results dialog headline and its all-runs row | `BANNER POINTS EARNED` · `Banner Points, all runs` | |

## Modifiers, buffs and debuffs shown to the player

| CATEGORY | CURRENT NAME | WHERE IT APPEARS | CURRENT DESCRIPTION TEXT | NEW NAME |
|---|---|---|---|---|
| Buff | **sheltered** | Tower panel subtitle, whenever a tower is inside a Tax Shelter | `· +30% sheltered` | |
| Buff stat row | **Nearby damage** | Tower panel, on a support tower | `+30%` (or `+30% → +47%` when upgrading) | |
| State | **DAD MODE** | HUD hero bar | see Hero abilities | |
...
| Button | **START RUN** | Title screen | — | |
| Button | **CREDITS** | Title screen | — | |
| Screen title | **YOUR LOADOUT** | Loadout screen | Subtext: `Drawn at random for this run.` | |
| Section heading | **SPECIALS** | Loadout screen | — | |
| Button | **BEGIN THE RUN** | Loadout screen | — | |
| Button | **REROLL (1 left)** | Loadout screen | — | |
| Button | **START WAVE n** / **WAVE n · Ns** | HUD | Early-start bonus appended when there is one | |
| Button | **BUILD** | Build menu | — | |
| Button | **UPGRADE · N** / **SPECIALIZE · N** / **SELL · N** | Tower panel | — | |
| Panel heading | **TIER n OF 3** / **<TOWER> — TIER 3** | Tower panel, specialize dialog | — | |
| Dialog | **PAUSED** | Pause dialog | `The wave is stopped. Nothing moves until you resume.` · RESUME · RESTART | |
| Dialog | **QUIT TO TITLE?** | Quit confirmation | `This run ends here. Towers, upgrades and peanuts are lost.` · KEEP PLAYING | |
| Dialog | **THE LINE HELD** | Results, on a win | Message behind it: `Filed on time.` · TRY AGAIN · QUIT TO TITLE | |
| Dialog | **THE LINE BROKE** | Results, on a loss | Message behind it: `Overrun.` | |
| Wave banner | **CLEARED** / **OVERRUN** | HUD | — | |
| Overlay | **SERVER NUKE** | Earned announcement and launch confirmation | — | |
| Toggle | **MUTED** | Audio button | — | |
| Credits | **TAP TO SKIP** | Credits roll | — | |

## Flavour text, tooltips and messages that carry the theme

| CATEGORY | CURRENT NAME | WHERE IT APPEARS | CURRENT DESCRIPTION TEXT | NEW NAME |
|---|---|---|---|---|
| Result verdict | Flawless | Results dialog subtitle | `Not one of them got through. Immaculate records.` | |
| Result verdict | Clean | Results dialog subtitle | `A few slipped past. Nothing an amendment will not cover.` | |
| Result verdict | Narrow | Results dialog subtitle | `Filed under duress, and filed.` | |
| Result verdict | Lost | Results dialog subtitle | `They reached the filing cabinet.` | |
| Win message | — | HUD message line | `Filed on time.` | |
