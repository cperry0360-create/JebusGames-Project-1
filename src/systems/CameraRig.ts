...
export interface CameraLimits {
  ...
  minZoom?: number
  /**
   * THERE IS NO BOUNDS MARGIN AND NO HUD BAND ALLOWANCE. Both were here, and
   * together they were the "black screen when scrolling" bug on every level:
   * `boundsMarginPx` (0 in `display.json`, so harmless) and `hudBandPx` (the
   * HUD's own band height, so up to 137 world px), handed to `centerRange` as
   * a margin on the camera CENTRE. A margin there does not create slack inside
   * the plate; it moves the wall outward and the view past the artwork. See
   * the header of `centerRange`, and `tools/harness/run.sh edges`.
   *
   * The clamp is now exactly "the view stays on the plate", with nothing to
   * tune. If an edge build pad is under the HUD, the answer is a camera
   * position or a zoom that clears it, which `tests/hudpads.test.ts` proves
   * exists for all 151 pads on all ten levels.
   */
...
export class CameraRig {
  ...
  update(dt: number): void {
    ...
    // No margin on either axis. The plate IS the world, so a centre range
    // that keeps the view inside the world is the whole clamp; see the note on
    // the removed limits above.
