...
export class DiagnosticsScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const y = H - 34
    const gap = 12
    // Three buttons, not four. The fourth cleared the seen-cutscene flags so
    // that every comic would play again; comics play every time now, so it
    // had nothing left to clear.
