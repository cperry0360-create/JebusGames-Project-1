...
| **Kenney — Impact Sounds, Interface Sounds, UI Audio, RPG Audio** | 32 of the 41 sound cues: tower fire, hits, deaths, clicks, errors, coins, building | Same mirror; `tools/getsfx.py` records which file became which cue | **CC0 1.0** — see `public/assets/audio/CREDITS.md` | No | Yes — same line |
...
- The **three voice lines** (`sfx-goblin-spawn`, `sfx-haymaker-voice`,
  `sfx-dadmode-voice`): recorded by Elijah for this project. Original work,
  no third-party licence involved. Measured from the files themselves: 44.1
  kHz mono, 1.44s / 0.95s / 2.2s, peaking at -1.4, -1.8 and -2.5 dBFS.
...
multi-second audio stings (generated by `tools/mksfx.py`), the three voice
lines, and all code.

### Contributors

Family, and what each of them actually did. All of it is original work owned by
the project; none of it carries an outside licence. Courtland, Elijah and Han
each have a card of their own in the credits roll.

| Who | Contribution | Where it shows up |
|---|---|---|
| **Cory** | Direction, design, art direction, balance, and the entire cast list joke | Everywhere. `credits.json` is almost entirely him, which is the joke |
| **Courtland** | Music supervision and soundtrack curation — the tracks were chosen, not stumbled into | The MUSIC section, and lead game tester in QA |
| **Elijah** | **Voice actor.** All three recorded lines: the goblin as the first enemy comes out of the arch, the Haymaker landing, and DAD MODE (`sfx-goblin-spawn`, `sfx-haymaker-voice`, `sfx-dadmode-voice`) | His own card in the AUDIO section, which names each line |
| **Han** | Craft services and catering | Her own card, and field research in QA |
